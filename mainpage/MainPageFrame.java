...
//	idDAO dao = new idDAO();
//	idDTO dto = new idDTO();
//	List<idDTO> dtolist = dao.selectAll();
//	
...
//	}
	...
	ImageIcon loginfin = new ImageIcon("login.png"); 
	JButton b_loginfin = new JButton(loginfin); // 로그아웃 버튼
	...
	public MainPageFrame() {
		...
		start();
		setVisible(onoff);
		// F5 키 이벤트를 감지하기 위해 KeyListener 추가
        addKeyListener(this);
	...
	private void init() {
		...
		p_mainTot.setBackground(color_beige);
...
//          System.out.println("F5 키가 눌렸습니다.");
	...
	private void changebutton() {
		 // 로그인 버튼을 로그인 완료 버튼으로 변경
//	    p_login.remove(b_login);
//	    p_login.add(b_loginfin);
	    // 회원가입 버튼을 빈칸으로 교체
//	    p_login.remove(b_join);
//	    p_login.add(b_loginfin); // 빈칸 추가
	    p_login.revalidate(); // 변경된 패널을 다시 그리도록 함
